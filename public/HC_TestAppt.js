    ...
    $("#ok-button").unbind().click({date: event.data.date}, function() {
        ...
        //}
         if(isNaN(count)) {
            ...
            new_event_json(time, count, date, day);
            date.setDate(day);
            init_calendar(date);
        }
    });
...
function delete_event(event) {
    ...
    $("#ok-delbutton").click(function(event) {
        ...
        $("#dialog").hide(250);
        $("#deldialog").hide(250);
        delete_event_json(event.time, event.count, event.date, event.day);
        $(".events-container").show(250);
...
function new_event_json(time, count, date, day) {
    var event = {
        "time": time,
        "invited_count": count,
...
/////////
///rmz:
function delete_event_json(time, count, date, day) {
    var event = {
        "time": time,
        "invited_count": count,
        "year": date.getFullYear(),
...
function show_events(events, month, day) {
    ...
    if(events.length===0) {
        ...
        for(var i=0; i<events.length; i++) {
            ...
            var event_count = $("<div class='event-count'>"+events[i]["invited_count"]+" Appointments <input type = 'radio' ></div>");
